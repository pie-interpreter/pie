...
class PHPPreTokenizer(object):
    ...
    def run(self):
        ...
        tokens.append(self.create_token(self.text[self.laststart: len(self.text)]));
        tokens.append(Token("EOF", "EOF", SourcePos(self.laststart, self.lineno,
                                                    self.columnno)))
    ...
    def create_token(self, text):
        ...
        token = Token(name, text, SourcePos(self.laststart, self.lineno, self.columnno))
